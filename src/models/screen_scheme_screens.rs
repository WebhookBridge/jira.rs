/*
 * The Jira Cloud platform REST API
 *
 * Jira Cloud platform REST API documentation
 *
 * The version of the OpenAPI document: 1001.0.0-SNAPSHOT
 * Contact: ecosystem@atlassian.com
 * Generated by: https://openapi-generator.tech
 */

/// ScreenSchemeScreens : The IDs of the screens for the screen types of the screen scheme.



#[derive(Clone, Debug, PartialEq, Default, Serialize, Deserialize)]
pub struct ScreenSchemeScreens {
    /// The ID of the create screen.
    #[serde(rename = "create", skip_serializing_if = "Option::is_none")]
    pub create: Option<i64>,
    /// The ID of the default screen. Required when creating a screen scheme.
    #[serde(rename = "default", skip_serializing_if = "Option::is_none")]
    pub default: Option<i64>,
    /// The ID of the edit screen.
    #[serde(rename = "edit", skip_serializing_if = "Option::is_none")]
    pub edit: Option<i64>,
    /// The ID of the view screen.
    #[serde(rename = "view", skip_serializing_if = "Option::is_none")]
    pub view: Option<i64>,
}

impl ScreenSchemeScreens {
    /// The IDs of the screens for the screen types of the screen scheme.
    pub fn new() -> ScreenSchemeScreens {
        ScreenSchemeScreens {
            create: None,
            default: None,
            edit: None,
            view: None,
        }
    }
}


