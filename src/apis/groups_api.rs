...
pub async fn bulk_get_groups(
    ...
    if let Some(ref local_var_str) = access_type {
        local_var_req_builder =
            local_var_req_builder.query(&[("accessType", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_str) = application_key {
        local_var_req_builder =
            local_var_req_builder.query(&[("applicationKey", &local_var_str.to_string())]);
    }
    if let Some(ref local_var_user_agent) = local_var_configuration.user_agent {
        local_var_req_builder =
            local_var_req_builder.header(reqwest::header::USER_AGENT, local_var_user_agent.clone());
