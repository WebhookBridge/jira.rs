...
pub async fn create_workflow(
    ...
    create_workflow_details: crate::models::CreateWorkflowDetails,
) -> Result<crate::models::WorkflowIds, Error<CreateWorkflowError>> {
...
pub async fn delete_inactive_workflow(
    ...
    let local_var_uri_str = format!(
        ...
        entityId = crate::apis::urlencode(entity_id)
    );
    let mut local_var_req_builder =
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<DeleteInactiveWorkflowError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
...
) -> Result<Vec<crate::models::DeprecatedWorkflow>, Error<GetAllWorkflowsError>> {
    let local_var_configuration = configuration;
...
) -> Result<crate::models::PageBeanWorkflow, Error<GetWorkflowsPaginatedError>> {
    let local_var_configuration = configuration;
    ...
    if let Some(ref local_var_str) = workflow_name {
        local_var_req_builder = match "multi" {
            "multi" => local_var_req_builder.query(
                &local_var_str
                    .into_iter()
                    .map(|p| ("workflowName".to_owned(), p.to_string()))
